"""
database.py
Модуль для работы с PostgreSQL базой данных ресторана
"""
...
class PostgreSQLDatabase:
    ...
    def get_menu_items(self, category_id: Optional[int] = None,
                       available_only: bool = True) -> List[Any]:
        """Получает блюда из меню"""
        from models import MenuItem

        query = """
            SELECT m.id, m.name, m.description, m.price, m.category_id, 
                   m.is_available, m.calories, m.cooking_time_minutes,
        ...
        with self.connection.cursor() as cursor:
            ...
            return [MenuItem(
                ...
                calories=row[6], cooking_time=row[7]
            ) for row in cursor.fetchall()]
